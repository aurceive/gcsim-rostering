...
func TestEditConfig_ReplacesWeaponAndStats_PreservesTail(t *testing.T) {
	...
	out, err := EditConfig(input, "diluc", "serpentspine", 1, "atk%=0.466 cr=0.311 cd=0.622")
	if err != nil {
...
func TestEditConfig_StatsHpTokenMismatch_NoReplace(t *testing.T) {
	...
	_, err := EditConfig(input, "diluc", "wgs", 1, "atk%=0.466 cr=0.311 cd=0.622")
	if err == nil {
...
func TestEditConfig_MainStatsTooShort_ReturnsError(t *testing.T) {
	...
	_, err := EditConfig(input, "diluc", "wgs", 1, "atk%=0.466 cr=0.311")
	if err == nil {
