...
Единица результата для merge/skip/частичного сохранения:

- одно оружие
- одно пробуждение
- один `substat_optimizer_variant`
...
Булево значение. Если задано `true`, то записи `weapon+refine+variant`, которые уже есть в базовой таблице
(`base_table_path` или существующий `output_table_path`), **не пересчитываются**.

Полезно для продолжения прерванного расчёта.

#### `trust_existing_results` (опционально)

Булево значение. Если задано `true`, то при merge существующий результат сохраняется,
**если новый не лучше** (по выбранному `target`) для той же записи `weapon+refine+variant`.

#### `ignore_existing_results` (опционально)

Булево значение. Если задано `true`, полностью игнорируются найденные ранее результаты
(автопоиск и существующий `output_table_path`). Несовместимо с `base_table_path`.
...
- `skip_existing_results`, merge и частичное сохранение тоже различают эти варианты отдельно
