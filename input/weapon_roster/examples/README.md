...
- `config.exemple.txt` — текстовый конфиг симуляции (формат gcsim/wfpsim). Это «шаблон» ротации/команды, который программа будет многократно запускать, **подменяя оружие, пробуждение и мейн-статы**.
- `roster_config.exemple.yaml` — YAML-настройки перебора (кого оптимизируем, какие мейн-статы перебирать, какой движок использовать и т.п.).
...
- допустимые значения в репозитории: `gcsim`, `wfpsim`, `custom`
...
Это обычный конфиг для движка (gcsim/wfpsim/custom) с ротацией, целями, опциями и т.д.
...
2) у оптимизируемого персонажа подменяет:
