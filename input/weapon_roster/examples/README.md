...
- `config.example.txt` — текстовый конфиг симуляции (формат gcsim/wfpsim). Это «шаблон» ротации/команды, который программа будет многократно запускать, **подменяя оружие, пробуждение и мейн-статы**.
